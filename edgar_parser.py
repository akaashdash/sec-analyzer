# Adapted from https://github.com/nlpaueb/edgar-crawler
...
def process_filing(content):
    ...
    doc_10k = remove_html_tables(doc_10k, is_html=is_html)

    json_content = {}
    for item_index in items_to_extract:
        json_content[f'item_{item_index}'] = ''

    text = strip_html(str(doc_10k))
...
def remove_html_tables(doc_10k, is_html):
    ...
    if is_html:
        ...
        for tbl in tables:
            ...
            tbl_text = clean_text(strip_html(str(tbl)))
...
def parse_item(text, item_index, next_item_list, positions):
    """
